"""
app.py  –  Buscador de Normativa Educativa (100% Semántico - Versión Ligera)
...
# ── CARGA DEL CEREBRO SEMÁNTICO EN CACHÉ ──
@st.cache_resource(show_spinner=False)
def load_rag_system():
    """Carga el modelo de lenguaje, el índice vectorial y los metadatos solo una vez."""
...
def main():
    ...
    if not INDEX_FILE.exists() or not METADATA_FILE.exists():
        st.error("⚠️ Faltan los archivos 'faiss_index.bin' o 'chunks_metadata.json'. Asegúrate de haberlos subido a GitHub.")
        st.stop()

    with st.spinner("Cargando base de datos semántica..."):
